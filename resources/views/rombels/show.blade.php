...
<div class="space-y-6" x-data="{ 
    ...
    confirmRemove(rombelId, siswaId, siswaNama) {
        Swal.fire({
            title: 'Keluarkan Siswa?',
            text: `Apakah Anda yakin ingin mengeluarkan ${siswaNama} dari rombel ini?`,
            icon: 'warning',
            ...
            if(result.isConfirmed) {
                document.getElementById('remove-form-' + siswaId).submit();
            }
        });
    },
    isMax: false,
                    ...
                    <tr class="hover:bg-sky-50/30 transition-colors group">
                        ...
                        <td class="px-8 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button type="button" @click="confirmRemove('{{ $rombel->id }}', '{{ $siswa->id }}', '{{ addslashes($siswa->nama) }}')" class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-3 py-1.5 rounded-lg transition-all border border-rose-100 cursor-pointer">
