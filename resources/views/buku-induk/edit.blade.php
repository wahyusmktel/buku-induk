        ...
        <form action="{{ route('buku-induk.update', $siswa->nisn) }}" method="POST" class="p-8">
            ...
            <div x-show="section === 'identitas'" x-transition>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @php
                        $fields = [
                            ['name' => 'no_induk', 'label' => 'Nomor Buku Induk', 'type' => 'text', 'value' => $bukuInduk->no_induk],
                            ['name' => 'nama_panggilan', 'label' => 'Nama Panggilan', 'type' => 'text', 'value' => $bukuInduk->nama_panggilan],
                            ['name' => 'bahasa_sehari_hari', 'label' => 'Bahasa Sehari-hari', 'type' => 'text', 'value' => $bukuInduk->bahasa_sehari_hari, 'placeholder' => 'contoh: Indonesia'],
                        ];
                    @endphp
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="golongan_darah" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                            ...
                            <option value="{{ $gol }}" {{ old('golongan_darah', $bukuInduk->golongan_darah) == $gol ? 'selected' : '' }}>{{ $gol }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="kewarganegaraan" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                            <option value="WNI" {{ old('kewarganegaraan', $bukuInduk->kewarganegaraan) == 'WNI' ? 'selected' : '' }}>WNI</option>
                            <option value="WNA" {{ old('kewarganegaraan', $bukuInduk->kewarganegaraan) == 'WNA' ? 'selected' : '' }}>WNA</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="bertempat_tinggal_dengan" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                            ...
                            <option value="{{ $opt }}" {{ old('bertempat_tinggal_dengan', $bukuInduk->bertempat_tinggal_dengan) == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="jml_saudara_tiri" min="0" value="{{ old('jml_saudara_tiri', $bukuInduk->jml_saudara_tiri ?? 0) }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="jml_saudara_angkat" min="0" value="{{ old('jml_saudara_angkat', $bukuInduk->jml_saudara_angkat ?? 0) }}"
                               class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                    </div>
                    ...
                    <div class="md:col-span-2 lg:col-span-3 space-y-1.5">
                        ...
                        <textarea name="riwayat_penyakit" rows="3" placeholder="Contoh: Tifus (2022), Bronkitis..."
                                  class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">{{ old('riwayat_penyakit', $bukuInduk->riwayat_penyakit) }}</textarea>
                    </div>

                    <div class="md:col-span-2 lg:col-span-3 space-y-1.5">
                        <label class="text-xs font-black text-slate-500 uppercase tracking-widest">Catatan Beasiswa</label>
            ...
            <div x-show="section === 'orang_tua'" x-transition>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-5">
                        ...
                        @php
                            $pFields = [
                                'tempat_lahir_' . $key => ['label' => 'Tempat Lahir', 'type' => 'text'],
                                'tanggal_lahir_' . $key => ['label' => 'Tanggal Lahir', 'type' => 'date'],
                                'agama_' . $key => ['label' => 'Agama', 'type' => 'select', 'options' => ['Islam','Kristen','Katolik','Hindu','Buddha','Konghucu','Lainnya']],
                                'kewarganegaraan_' . $key => ['label' => 'Kewarganegaraan', 'type' => 'select', 'options' => ['WNI', 'WNA']],
                                'alamat_' . $key => ['label' => 'Alamat', 'type' => 'textarea'],
                            ];
                        @endphp
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="{{ $name }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                                ...
                                <option value="{{ $opt }}" {{ old($name, $bukuInduk->$name) == $opt ? 'selected' : '' }}>{{ $opt }}</option>
                                @endforeach
                            </select>
                            @elseif($meta['type'] === 'textarea')
                            <textarea name="{{ $name }}" rows="2" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">{{ old($name, $bukuInduk->$name) }}</textarea>
                            @else
                            <input type="{{ $meta['type'] }}" name="{{ $name }}" value="{{ old($name, $meta['type'] === 'date' ? ($bukuInduk->$name?->format('Y-m-d') ?? '') : $bukuInduk->$name) }}"
                                   class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                            @endif
                        </div>
                        @endforeach
                ...
                <div class="mt-10 pt-8 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        @foreach([
                            'nama_wali_bi' => 'Nama Wali',
                            'hubungan_wali' => 'Hubungan dengan Siswa',
                            'pekerjaan_wali_bi' => 'Pekerjaan',
                            'pendidikan_wali_bi' => 'Pendidikan Terakhir',
                            'telp_wali_bi' => 'No. Telepon',
                        ] as $name => $label)
                        <div class="space-y-1.5">
                            <label class="text-xs font-black text-slate-500 uppercase tracking-widest">{{ $label }}</label>
                            <input type="text" name="{{ $name }}" value="{{ old($name, $bukuInduk->$name) }}"
                                   class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
                        </div>
                        @endforeach
                        <div class="md:col-span-2 lg:col-span-3 space-y-1.5">
            ...
            <div x-show="section === 'masuk_keluar'" x-transition>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-5">
                        ...
                        @foreach(['tgl_lulus' => ['Tanggal Lulus', 'date'], 'no_ijazah' => ['Nomor Ijazah', 'text'], 'lanjut_ke' => ['Melanjutkan ke Sekolah', 'text'], 'tgl_keluar' => ['Tanggal Keluar', 'date'], 'alasan_keluar' => ['Alasan Keluar', 'text']] as $name => [$label, $type])
                        <div class="space-y-1.5">
                            <label class="text-xs font-black text-slate-500 uppercase tracking-widest">{{ $label }}</label>
                            <input type="{{ $type }}" name="{{ $name }}" value="{{ old($name, $type === 'date' ? ($bukuInduk->$name?->format('Y-m-d') ?? '') : $bukuInduk->$name) }}"
                                   class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 font-bold text-slate-700">
